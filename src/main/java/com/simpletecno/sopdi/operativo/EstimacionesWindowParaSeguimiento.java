...
public class EstimacionesWindowParaSeguimiento extends Window {
    ...
    protected static final String CCOSTO_PROPERTY        = "Centro Costo";
...
//        titleLayout.setWidth("100%");
    ...
    private void buscarEstimacion() {
        String queryString = "Select Est.IdProveedor, Est.Estatus, Emp.Empresa, Est.Moneda ";
        queryString += " From  estimacion Est";
        queryString += " Inner Join contabilidad_empresa Emp On Emp.IdEmpresa = Est.IdEmpresa";
        queryString += " Where Est.EstimacionId = " + estimacionId;
        ...
        try {
            ...
            if(rsRecords.next()) {              
                proveedorId = rsRecords.getString("IdProveedor");
                empresaNombre = rsRecords.getString("Empresa");
                estatus = rsRecords.getString("Estatus");
                moneda = rsRecords.getString("Moneda");
            }
            
        } catch (Exception ex1) {
            System.out.println("Error al buscar estimaciones " + ex1.getMessage());
            Notification.show("Error al buscar registros de estimaciones detalle", Notification.Type.ERROR_MESSAGE);
            ex1.printStackTrace();
        }
    ...
    private void llenarComboProveedor() {
        ...
        queryString += " WHERE (N0 = 6 or N0 = 7)";
        queryString += " AND N1 <> 8";
        if(!proveedorId.equals("0")) {
            queryString += " And IdProveedor = " + proveedorId;
        }
        queryString += " Order By Proveedor ";
        ...
        try {
            ...
            while (rsRecords.next()) {              
                ...
                proveedorCbx.setItemCaption(rsRecords.getString("IDProveedor"), rsRecords.getString("Proveedor"));
            }
    ...
    private void createGrids() {
        ...
        fromContainer.addContainerProperty(CCOSTO_PROPERTY,         Integer.class, null);
        fromContainer.addContainerProperty(IDEX_PROPERTY,           String.class, null);
        fromContainer.addContainerProperty(PROJECT_PROPERTY,        Integer.class, null);
        ...
        toContainer.addContainerProperty(CCOSTO_PROPERTY,         Integer.class, null);
        toContainer.addContainerProperty(IDEX_PROPERTY,           String.class, null);
        toContainer.addContainerProperty(PROJECT_PROPERTY,        Integer.class, null);
...
//System.out.println("\n\n"+queryString);
        ...
        try {
            ...
            Logger.getLogger(EstimacionesWindowParaSeguimiento.class.getName()).log(Level.SEVERE, null, ex);
...
**/        
        ...
        fromGridFooter.getCell(TOTAL_PROPERTY).setText("0.00");
        toGridFooter.getCell(TOTAL_PROPERTY).setText("0.00");
        ...
        try {
            ...
            if(rsRecords.next()) { //  encontrado
                ...
                do {
                    ...
                    queryString = " Select IfNull(Sum(TotalEstimacion),0) TotalCobrado, ";
                    queryString += " IfNull(Sum(Provision),0) TotalProvision, Estimacion ";
                    queryString += " From estimacion";
                    queryString += " Where IdProveedor = " + rsRecords.getString("IdProveedor");
                    queryString += " And   IdCC        = " + rsRecords.getString("IdCC");
                    queryString += " And   IdEmpresa   = " + this.empresa;
                    queryString += " And   Moneda      = '" + rsRecords.getString("Moneda") + "'";
                    queryString += " And   IDEX        = " + rsRecords.getString("IDEX");
                    queryString += " And   IdProject   = " + rsRecords.getString("IdProject");
...
//System.out.println("\nQuerySaldoEstimacion="+queryString);
                    ...
                    if(rsRecords1.next()) { //  encontrado
                        saldo = saldo - rsRecords1.getDouble("TotalCobrado");
                    }
                    ...
                    fromContainer.getContainerProperty(itemId, PROJECT_PROPERTY).setValue(rsRecords.getInt("IdProject"));
                    fromContainer.getContainerProperty(itemId, MONEDA_PROPERTY).setValue(rsRecords.getString("Moneda"));
                    fromContainer.getContainerProperty(itemId, TOTAL_PROPERTY).setValue(moneyFormat.format(saldo));
                    fromContainer.getContainerProperty(itemId, TOTALSF_PROPERTY).setValue(saldo);
...
/****
        if(!estatus.equals("PAGADA") && !estatus.equals("APROBADA")) {  //estimacion pendiente de CONFIRMAR, APROBAR Y PAGAR, se pueden seleccionar IDEX
            for (Object itemId: selection.getSelectedRows()) {
                total = total.add(new BigDecimal(
                        Double.parseDouble(
                            String.valueOf(fromContainer.getContainerProperty(itemId, TOTALSF_PROPERTY).getValue())
                    )
                ));
                provision = provision.add(new BigDecimal(
                        Double.parseDouble(
                            String.valueOf(fromContainer.getContainerProperty(itemId, PROVISIONSF_PROPERTY).getValue())
                    )
                ));
                totalest = totalest.add(new BigDecimal(
                        Double.parseDouble(
        ...
        else { // la estimación ya está pagada, se totalizan todos los IDEX relacionados
...
****/ 
        fromGridFooter.getCell(TOTAL_PROPERTY).setText(moneyFormat.format(total));
        fromGridFooter.getCell(PROVISION_PROPERTY).setText(moneyFormat.format(provision));
        fromGridFooter.getCell(TOTALEST_PROPERTY).setText(moneyFormat.format(totalest));
        if(!estatus.equals("PAGADA") && !estatus.equals("APROBADA")) {  //estimacion pendiente de pago, se pueden seleccionar IDEX
