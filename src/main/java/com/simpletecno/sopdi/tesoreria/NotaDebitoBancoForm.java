 ...
 * @author user
 */
public class NotaDebitoBancoForm extends Window {

    static final String NIT_PROPERTY = "NIT";
    static final String OTROS_COBROS_BANCARIOS = "OTROS COBROS BANCARIOS";
    static final String MORA = "MORA";
    static final String DEBITO_CHEQUE_DEVUELTO = "DEBITO POR CHEQUE DEVUELTO";
    ...
    public HorizontalLayout crearComponentes() {
        ...
        horizontalLayout.setExpandRatio(rightVerticalLayout, 2);

        tipoEgresoCbx = new ComboBox("Tipo de Egreso :");
        tipoEgresoCbx.setWidth("100%");
        tipoEgresoCbx.setFilteringMode(FilteringMode.CONTAINS);
        tipoEgresoCbx.setInvalidAllowed(false);
        tipoEgresoCbx.setNewItemsAllowed(false);
        tipoEgresoCbx.setNullSelectionAllowed(false);
        tipoEgresoCbx.addItem(OTROS_COBROS_BANCARIOS);
        tipoEgresoCbx.addItem(MORA);
        tipoEgresoCbx.addItem(DEBITO_CHEQUE_DEVUELTO);
        tipoEgresoCbx.select(tipo);
        tipoEgresoCbx.setReadOnly(true);
        ...
        medioCbx.select("NOTA DE DEBITO");
        ...
        montoTxt.addValueChangeListener(new Property.ValueChangeListener() {
            public void valueChange(Property.ValueChangeEvent event) {
               ...
               debe2Txt.setValue(montoTxt.getDoubleValueDoNotThrow());
        ...
        leftVerticalLayout.addComponent(tipoEgresoCbx);
    ...
    public void llenarComboProveedor() {
        ...
        proveedorCbx.setCaption("Proveedor :");
        if (tipoEgresoCbx.getValue().equals(DEBITO_CHEQUE_DEVUELTO)) {
    ...
    public void llenarComboCuentaContable() {
        ...
        try {
            ...
            System.out.println("Error al combo cuentas contables ID4 = 11: " + ex1.getMessage());
            ex1.printStackTrace();
        }
    }

    public void generarPartidaEgreso() {
        ...
        if (tipoEgresoCbx.getValue().equals(DEBITO_CHEQUE_DEVUELTO)) {
            ...
            cuentaContable2Cbx.select(((SopdiUI) UI.getCurrent()).cuentasContablesDefault.getServiciosBancos());
        }
        haber2Txt.setReadOnly(true);

        cuentaContable3Cbx.setReadOnly(false);
        cuentaContable3Cbx.select(null);
        cuentaContable3Cbx.setReadOnly(true);
...
//            System.out.println("Hay "+dias+" dias de diferencia");
        ...
        if (numeroTxt.getValue().isEmpty()) {
            Notification.show("Por favor, ingreso un número de documento...", Notification.Type.ERROR_MESSAGE);
            numeroTxt.focus();
            return;
        }
        ...
        if (haber1Txt.getDoubleValueDoNotThrow() != montoTxt.getDoubleValueDoNotThrow()) {
            Notification.show("La partida esta descuadrada. Por favor, revise las cantidades..", Notification.Type.ERROR_MESSAGE);
            haber1Txt.focus();
            return;
        }

        if ((debe2Txt.getDoubleValueDoNotThrow() + debe3Txt.getDoubleValueDoNotThrow()) != montoTxt.getDoubleValueDoNotThrow()) {
            Notification.show("La partida esta descuadrada. Por favor, revise las cantidades..", Notification.Type.ERROR_MESSAGE);
            debe2Txt.focus();
            return;
        }
        ...
        if (cuentaContable3Cbx.getValue() != null) {
            if ((debe3Txt.getDoubleValueDoNotThrow() + haber3Txt.getDoubleValueDoNotThrow()) == 0) {
                Notification.show("Por favor, ingreso monto del DEBE...", Notification.Type.ERROR_MESSAGE);
                debe3Txt.focus();
                return;
            }
        }
       ...
       // }
        ...
        queryString += "," + cuentaContable1Cbx.getValue(); //bancos
        ...
        Logger.getLogger(this.getClass().getName()).log(Level.INFO, "queryString Egreso Bancos cobro = " + queryString);
        ...
        try {
            ...
            System.out.println("Error al insertar un egreso por cobro de bancos : " + ex1.getMessage());
            ex1.printStackTrace();
