 ...
 * @author user
 */
public class IngresoAnticipoEmpresaRelacionadaForm extends Window {
    ...
    String noCuentaAcreedoresProveedorSeleccionado;
    String noCuentaAnticiposXLiquidarProveedorSeleccionado;
    String idEmpresaProveedorSeleccionado;
    ...
    public HorizontalLayout crearComponentes() {
        ...
        anticiposCbx = new ComboBox("ANTICIPOS OTORGADOS :");
        anticiposCbx.setWidth("100%");
        anticiposCbx.setFilteringMode(FilteringMode.CONTAINS);
        anticiposCbx.setInvalidAllowed(false);
        anticiposCbx.setNewItemsAllowed(false);
        anticiposCbx.setNullSelectionAllowed(false);
        anticiposCbx.addContainerProperty("SALDO", String.class, "");
        anticiposCbx.addContainerProperty("IDNOMENCLATURA", String.class, "");
        anticiposCbx.addValueChangeListener((event) -> {
            if (anticiposCbx.getValue() != null) {
                montoTxt.setValue(Double.valueOf(String.valueOf(anticiposCbx.getContainerProperty(anticiposCbx.getValue(), "SALDO").getValue())));
                //cuentaContable2Cbx.select(String.valueOf(anticiposCbx.getContainerProperty(anticiposCbx.getValue(), "IDNOMENCLATURA").getValue()));
            }
//System.out.println(anticiposCbx.getContainerProperty(anticiposCbx.getValue(), "IDNOMENCLATURA").getValue());
        }
        );

        proveedorCbx = new ComboBox("Empresa Relacionada");
        proveedorCbx.addContainerProperty(NIT_PROPERTY, String.class, "");
        proveedorCbx.addContainerProperty("IDEMPRESA", String.class, "");
        ...
        proveedorCbx.addValueChangeListener((event) -> {
            if (proveedorCbx.getValue() != null) {
                llenarCuentaAcreedoresYLiquidar();
                llenarComboAnticipos();
            }
        });
        ...
        rightVerticalLayout.addComponent(anticiposCbx);
    ...
    public void llenarCuentaAcreedoresYLiquidar() {
        ...
        try {
            ...
            idEmpresaProveedorSeleccionado = "";
                ...
                if (rsRecords2.next()) {
                    ...
                    if (rsRecords3.next()) {
                        ...
                        do {
                            ...
                            if (rsRecords3.getString("NoCuenta").equals(noCuentaAcreedoresProveedorSeleccionado)) {

                                cuentaAcreedoresProveedorSeleccionado = rsRecords3.getString("IdNomenclatura");
                                cuentaContable2Cbx.select(cuentaAcreedoresProveedorSeleccionado);
                            }
                            if (rsRecords3.getString("NoCuenta").equals(noCuentaAnticiposXLiquidarProveedorSeleccionado)) {

                                cuentaAnticipoXLiquidarProveedorSeleccionado = rsRecords3.getString("IdNomenclatura");
                            }
                        } while (rsRecords3.next());
                    }
                    System.out.println("CUENTA DE ACREEDORES /n" + cuentaAcreedoresProveedorSeleccionado);
                    System.out.println("CUENTA ANTICIPOS X LIQUIDADOR PROVEEDOR /n" + cuentaAnticipoXLiquidarProveedorSeleccionado);
                }
            }
        } catch (Exception ex) {
            System.out.println("Error al intentar consultar la cuenta de acreedores" + ex);
        }
    }
    ...
    public void llenarComboProveedor() {
        ...
        try {
            ...
            while (rsRecords.next()) {  /// encontrado
                ...
                proveedorCbx.getItem(rsRecords.getString("IDProveedor")).getItemProperty("IDEMPRESA").setValue(rsRecords.getString("IDEMPRESA"));
            }
    ...
    public void llenarComboAnticipos() {   //tambien incluye los traslados...

        if (proveedorCbx == null) {
            return;
        }
        if (proveedorCbx.size() == 0) {
            return;
        }
...
//        System.out.println("Mostar anticipo " + queryString);
        try {
            ...
            while (rsRecords.next()) {
                ...
                anticiposCbx.getItem(rsRecords.getString("CodigoCC")).getItemProperty("SALDO").setValue(rsRecords.getString("SaldoAnticipo"));
                anticiposCbx.getItem(rsRecords.getString("CodigoCC")).getItemProperty("IDNOMENCLATURA").setValue(rsRecords.getString("IdNomenclatura"));
            }
            if (rsRecords.first()) {
                anticiposCbx.select(rsRecords.getString("CodigoCC"));
            }

        } catch (Exception ex1) {
            System.out.println("Error al listar de ANTICIPOS OTORGADOS U OTROS INGRESOS. " + ex1.getMessage());
            ex1.printStackTrace();
        }
    }
    ...
    public void generarPartidaIngreso() {
...
//            System.out.println("Hay " + dias + " dias de diferencia");
        ...
        if (proveedorCbx.getValue() == null) {
            Notification.show("Por favor, seleccione un proveedor (el banco)..", Notification.Type.ERROR_MESSAGE);
            proveedorCbx.focus();
            return;
        }
...
//        System.out.println("queryString Ingreso Bancos ANTICIPO DE EMPRESA RELACIONDA = " + queryString);
        Logger.getLogger(this.getClass().getName()).log(Level.INFO, "queryString Ingreso Bancos ANTICIPO EMPRESA RELACIONADA = " + queryString);
        ...
        try {
            ...
            System.out.println("Error al insertar ingreso a bancos por anicipo de empresa relacionada : " + ex1.getMessage());
            ex1.printStackTrace();
