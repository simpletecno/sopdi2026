...
public class IngresoDepositoVentaMonedaForm extends Window {
    ...
    public IngresoDepositoVentaMonedaForm(String idDeposito) {
        ...
        if (!idDeposito.trim().isEmpty()) {
            llenarDatos();
        } else {
            generarPartidaDepositoVentaMoneda();
        }
    ...
    public HorizontalLayout crearComponentes() {
        ...
        tipoIngresoCbx.addItem("DEPOSITO POR VENTA DE MONEDA");
        tipoIngresoCbx.select("DEPOSITO POR VENTA DE MONEDA");
    ...
    public void generarPartidaDepositoVentaMoneda() {

        monedaCbx.select("QUETZALES");
        cuentaContable1Cbx.select(((SopdiUI) mainUI).cuentasContablesDefault.getBancosMonedaLocal());
        cuentaContable2Cbx.select(((SopdiUI) mainUI).cuentasContablesDefault.getEfectivoEnTransito());
        cuentaContable3Cbx.select(((SopdiUI) mainUI).cuentasContablesDefault.getDiferencialCambiario());
        ...
        try {
            ...
            while (rsRecords.next()) {  // encontrado                               
                ...
                if (contador == 0) {
                    ...
                    medioCbx.setValue(rsRecords.getString("TipoDocumento"));
                    numeroTxt.setValue(rsRecords.getString("NumeroDocumento"));
                    montoTxt.setValue(rsRecords.getDouble("Debe"));
                    monedaCbx.setValue(rsRecords.getString("MonedaDocumento"));
                    tipoCambioTxt.setValue(rsRecords.getString("TipoCambio"));
                ...
                if (contador == 1) {
                    cuentaContable2Cbx.setValue(rsRecords.getString("IdNomenclatura"));
                    haber2Txt.setValue(rsRecords.getDouble("Haber"));
                }
                ...
                if (contador == 2) {
                    ...
                    cuentaContable2Cbx.setValue(rsRecords.getString("IdNomenclatura"));
                    debe3Txt.setValue(rsRecords.getDouble("Debe"));
                    haber3Txt.setValue(rsRecords.getDouble("Haber"));
                }
                contador += 1;
