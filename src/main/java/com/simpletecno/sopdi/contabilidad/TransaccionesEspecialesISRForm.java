 ...
 * @author user
 */
public class TransaccionesEspecialesISRForm extends Window {
    ...
    public TransaccionesEspecialesISRForm(
            String empresa,
            String codigoPartida,
            String tipoDocumento,
...
/*  ---------------- Verificar Serie ----------------   */
        if(serieTxt.getValue().isEmpty() || serieTxt.getValue() == null){
            Notification.show("La Contantncia debe de Tener Serie.", Notification.Type.ERROR_MESSAGE);
            System.out.println("ERROR contancia ISR: Falta Serie");
            return false;
        }

/*  ---------------- Verificar Numero ----------------   */
        if(numeroTxt.getValue().isEmpty() || numeroTxt.getValue() == null) {
            Notification.show("La Contantncia debe de Tener Numero.", Notification.Type.ERROR_MESSAGE);
            System.out.println("ERROR contancia ISR: Falta Numero");
            return false;
        }
...
/*  ---------------- Verificar Numero y Serie ----------------   */
        ...
        try {
            ...
            if (rsRecords.next()) { //  encontrado
                ...
                System.out.println("ERROR Serie, Numero ISR: Numero : " + numeroTxt.getValue() + " | Serie : " + serieTxt.getValue());
                return false;
            }
    ...
    public void updatePartidaTransaccion() {
        ...
        for (Object itemId : container.getItemIds()) {
            ...
            totalDebe = totalDebe.add(new BigDecimal(Double.parseDouble(String.valueOf(debeValue))).setScale(2, RoundingMode.HALF_UP));
        ...
        if (editar == 2) { /// editar partida
            ...
            try {
                ...
                System.out.println("ERROR, BUSCAR ULTIMO CODIGO PARTIDA" + ex1.getMessage());
                ex1.printStackTrace();
            }
        }
