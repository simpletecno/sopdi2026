    ...
    public void generarTable(String codigoCC) {
        ...
        if (tipoDocumento.equals("TRANSACCION ESPECIAL")) {
            ...
            if (tipoDocumento.equals("CONSTANCIA ISR COMPRA")) {
                queryString += "AND contabilidad_nomenclatura_empresa.IdNomenclatura IN ( " + ((SopdiUI) UI.getCurrent()).cuentasContablesDefault.getProveedores() + ", " + ((SopdiUI) UI.getCurrent()).cuentasContablesDefault.getAnticiposProveedor() + ") ";
        ...
        try {
            ...
            for (Object itemId : container.getItemIds()) {
                ...
                if( monedaCbx.getValue().equals("DOLARES") && regularizarChb.getValue()) {
                    queryString += ", 0"; //debe
                    queryString += ", 0"; //haber
                    queryString += "," + item.getItemProperty("DEBE").getValue(); //DEBE QUETZALES
                    queryString += "," + item.getItemProperty("HABER").getValue(); //HABER QUETZALES
                } else {
